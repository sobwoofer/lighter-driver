...
void button_save_and_reboot(String title){
    page_content += F("<div id=\"messegeResult\"></div><input id=\"button\" type=\"button\" class=\"btn btn-");
    page_content += F("primary");
    page_content += F(" btn-block\" onclick=\"sendForm('formMain', '/post');reboot();\" value=\"");
    page_content += title;
    page_content += F("\"><br>");
...
void input_select(String id, String name, bool refresh, String title, String html_class = "") {
    int i = 0;
    page_content += F("<div class=\"input-group mb-3 ");
    page_content += html_class;
    page_content += F("\"><div class=\"input-group-prepend\"><label class=\"input-group-text\">");
    page_content += title;
